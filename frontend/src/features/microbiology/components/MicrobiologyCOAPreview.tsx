...
export default function MicrobiologyCOAPreview() {
  ...
  const { data: coa, isLoading: coaLoading } = useQuery<COA>({
    queryKey: ['microbiology-coa', unitId],
    queryFn: async () => {
      const response = await apiClient.get(`/microbiology-coa/${unitId}/`);
      return response.data;
    },
  });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="print:hidden fixed top-4 right-4 z-50 flex gap-2">
        <button
          ...
          className="bg-purple-600 text-white rounded font-semibold hover:bg-purple-700 transition shadow-lg flex items-center justify-center gap-1"
        >
      ...
      <div className="max-w-4xl mx-auto p-8">
        ...
        >
          ...
          <header style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{
                ...
                MICROBIOLOGY DIAGNOSTICS - Certificate of Analysis
              </div>
            </div>
            <div style={{
              marginLeft: 'auto',
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div>{unit.sample_type?.join(', ') || 'N/A'}</div>

                <div style={{ fontWeight: '600', color: '#4b5563' }}>Batch No:</div>
                <div>{unit.microbiology_data?.batch_no || 'N/A'}</div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ fontWeight: '600', color: '#4b5563' }}>Fumigation:</div>
                <div>{unit.microbiology_data?.fumigation || 'N/A'}</div>
              </div>
            </div>
          </div>
          ...
          {diseases.map(disease => (
            <div key={disease} style={{ marginBottom: '16px' }}>
              <div style={{
                fontSize: '14px',
                fontWeight: '700',
                marginBottom: '8px',
              ...
              <table style={{
                ...
                <tbody>
                  {indexList.map(index => {
                    const result = coa.test_results[disease]?.[index] || 'NA';
                    return (
                      <tr key={index}>
                        <td style={{
                          border: '1px solid #d1d5db',
