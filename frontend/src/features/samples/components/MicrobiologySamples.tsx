...
export const MicrobiologySamples = () => {
  ...
  useEffect(() => {
    if (unitRows.length > 0 && lastPageLoaded) {
      ...
      if (!targetRow) {
        const savedUnitId = localStorage.getItem('microbiology_selected_unit');
        if (savedUnitId) {
            ...
            {filteredRows.map((row: UnitRow) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-purple-700 text-base">{row.unitCode}</span>
                    <span
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  ...
                  <div><span className="text-gray-500 text-xs">Batch</span><p className="font-medium text-gray-800 truncate">{row.batchNo ?? '-'}</p></div>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-4 text-xs">
                    <div className="text-center">
                      <span className="block font-bold text-gray-700">{row.visibleSubSamples > 0 ? row.visibleSubSamples : (row.samplesNumber ?? '-')}</span>
                      <span className="text-gray-500">Sub-Samples</span>
                    </div>
                    <div className="text-center">
                      <span className="block font-bold text-purple-600">{row.visibleTestsCount > 0 ? row.visibleTestsCount : '-'}</span>
