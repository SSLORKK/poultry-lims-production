...
interface UnitRow {
  ...
  sampleType: string;
  status: string;
  samplesNumber: number | null;  // This is sub-samples count
  extraction: number | null;  // This is samples count (No. Samples)
  detection: number | null;  // Detection value - Total Tests = Sum of Detection values
  diseases: string;
  kitType: string;
...
export const PCRSamples = () => {
  ...
  const unitRows: UnitRow[] = useMemo(() => {
    ...
    return rows.sort((a, b) => {
      // Extract numeric parts from unit codes for proper numeric sorting
  ...
  useEffect(() => {
    if (unitRows.length > 0 && lastPageLoaded) {
      ...
      if (!targetRow) {
        const savedUnitId = localStorage.getItem('pcr_selected_unit');
        if (savedUnitId) {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'PCR Samples');

      const fileName = `pcr_samples_export_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      link.setAttribute('download', `pcr_samples_export_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6 pb-20 lg:pb-6">
      ...
      {(selectedCompanies.length > 0 || selectedFarms.length > 0 || selectedFlocks.length > 0 ||
        ...
        selectedDiseases.length > 0 || selectedKitTypes.length > 0 || selectedTechnicians.length > 0 ||
        selectedExtractionMethods.length > 0 || startDate || endDate) && (
          <div className="mb-4 flex flex-wrap gap-2">
            ...
            {selectedFlocks.map((flock) => (
              <span key={flock} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-yellow-100 text-yellow-800 rounded-full">
                ...
                <button
                  ...
                  aria-label={`Remove ${flock} filter`}
                >
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
            ...
            {selectedSampleTypes.map((sampleType) => (
              <span key={sampleType} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-teal-100 text-teal-800 rounded-full">
                ...
                <button
                  onClick={() => setSelectedSampleTypes(selectedSampleTypes.filter(st => st !== sampleType))}
                  className="ml-1 text-teal-600 hover:text-teal-800"
                  aria-label={`Remove ${sampleType} filter`}
                >
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
            ...
            {selectedHouses.map((house) => (
              <span key={house} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-cyan-100 text-cyan-800 rounded-full">
                ...
                <button
                  ...
                  aria-label={`Remove ${house} filter`}
                >
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
            ...
            {selectedDiseases.map((disease) => (
              <span key={disease} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-red-100 text-red-800 rounded-full">
                ...
                <button
                  ...
                  aria-label={`Remove ${disease} filter`}
                >
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
            ...
            {selectedKitTypes.map((kitType) => (
              <span key={kitType} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-violet-100 text-violet-800 rounded-full">
                <span className="font-medium">Kit Type:</span>
                <span>{kitType}</span>
                <button
                  onClick={() => setSelectedKitTypes(selectedKitTypes.filter(k => k !== kitType))}
                  className="ml-1 text-violet-600 hover:text-violet-800"
                  aria-label={`Remove ${kitType} filter`}
                >
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
            ...
            {selectedTechnicians.map((technician) => (
              <span key={technician} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-amber-100 text-amber-800 rounded-full">
                <span className="font-medium">Technician:</span>
                <span>{technician}</span>
                <button
                  onClick={() => setSelectedTechnicians(selectedTechnicians.filter(t => t !== technician))}
            ...
            {selectedExtractionMethods.map((method) => (
              <span key={method} className="inline-flex items-center gap-1 px-3 py-1 text-sm bg-emerald-100 text-emerald-800 rounded-full">
                <span className="font-medium">Extraction:</span>
                <span>{method}</span>
                <button
                  onClick={() => setSelectedExtractionMethods(selectedExtractionMethods.filter(m => m !== method))}
      ...
      {filterPanelOpen && (
        <div className="mb-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            {/* Diseases Multi-Select Dropdown */}
            <div>
              <div className="flex items-center justify-between mb-2">
            ...
            {/* Kit Type Multi-Select Dropdown */}
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs font-medium text-gray-700">Kit Types</label>
                {uniqueKitTypes.length > 0 && (
                  <button
                    onClick={() => setSelectedKitTypes(selectedKitTypes.length === uniqueKitTypes.length ? [] : uniqueKitTypes)}
                    className="text-xs text-blue-600 hover:text-blue-800 font-medium"
                  >
                    {selectedKitTypes.length === uniqueKitTypes.length ? 'Deselect All' : 'Select All'}
                  </button>
                )}
              </div>
              <div className="border border-gray-300 rounded p-2 max-h-32 overflow-y-auto bg-white">
                  ...
                  uniqueKitTypes.map((kitType) => (
                    <label key={kitType} className="flex items-center py-1 px-1 hover:bg-gray-50 cursor-pointer rounded">
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedKitTypes([...selectedKitTypes, kitType]);
                          } else {
                            setSelectedKitTypes(selectedKitTypes.filter(k => k !== kitType));
                          }
                        }}
                        className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                      />
            ...
            {/* Extraction Method Multi-Select Dropdown */}
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs font-medium text-gray-700">Extraction Methods</label>
                {uniqueExtractionMethods.length > 0 && (
                  <button
                    onClick={() => setSelectedExtractionMethods(selectedExtractionMethods.length === uniqueExtractionMethods.length ? [] : uniqueExtractionMethods)}
                    className="text-xs text-blue-600 hover:text-blue-800 font-medium"
                  >
                    {selectedExtractionMethods.length === uniqueExtractionMethods.length ? 'Deselect All' : 'Select All'}
                  </button>
                )}
              </div>
              <div className="border border-gray-300 rounded p-2 max-h-32 overflow-y-auto bg-white">
                {uniqueExtractionMethods.length === 0 ? (
                  <div className="text-xs text-gray-500 py-1 px-2">No extraction methods available</div>
                ) : (
                  uniqueExtractionMethods.map((method) => (
                    <label key={method} className="flex items-center py-1 px-1 hover:bg-gray-50 cursor-pointer rounded">
                      <input
                        type="checkbox"
                        checked={selectedExtractionMethods.includes(method)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedExtractionMethods([...selectedExtractionMethods, method]);
                          } else {
                            setSelectedExtractionMethods(selectedExtractionMethods.filter(m => m !== method));
                          }
                        }}
                        className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                      />
      ...
      {selectedRow && (
        <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl flex items-center justify-between shadow-sm animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-sm font-bold text-blue-900">{selectedRow.unitCode}</span>
              <span className="text-xs text-blue-600 ml-2">• {selectedRow.company} - {selectedRow.farm}</span>
            </div>
          </div>
      ...
      {filteredRows.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 text-lg mb-4">
            {unitRows.length === 0 ? 'No PCR samples registered yet' : 'No samples match your filters'}
          </p>
          <button
            onClick={clearFilters}
            ...
            {filteredRows.map((row: UnitRow) => (
              <div
                ...
                className={`bg-white rounded-xl shadow-sm border-2 p-4 cursor-pointer transition-all active:scale-[0.98] ${
                  selectedRow?.unitId === row.unitId ? 'border-blue-500 bg-blue-50/50' : 'border-gray-100'
                }`}
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    {(editedSampleIds.has(row.sampleId) || editedUnitIds.has(row.unitId)) && (
                      <span className="w-5 h-5 bg-orange-500 rounded flex items-center justify-center">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  ...
                  <div><span className="text-gray-500 text-xs">Kit Type</span><p className="font-medium text-gray-800 truncate">{row.kitType}</p></div>
                  <div><span className="text-gray-500 text-xs">Technician</span><p className="font-medium text-gray-800 truncate">{row.technicianName}</p></div>
                  <div className="col-span-2"><span className="text-gray-500 text-xs">Diseases</span><p className="font-medium text-gray-800 text-xs line-clamp-2">{row.diseases}</p></div>
                </div>
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-4 text-xs">
                    <div className="text-center"><span className="block font-bold text-gray-700">{row.extraction ?? '-'}</span><span className="text-gray-500">Sub</span></div>
                    <div className="text-center"><span className="block font-bold text-blue-600">{row.detection ?? '-'}</span><span className="text-gray-500">Tests</span></div>
                  </div>
                  {row.notes && <button onClick={(e) => { e.stopPropagation(); setNoteDialog({ open: true, note: row.notes }); }} className="px-3 py-1.5 text-blue-600 text-xs font-medium bg-blue-50 rounded-lg">Note</button>}
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody className="bg-white">
                {filteredRows.map((row: UnitRow) => (
                  ...
                  >
                    ...
                    <td className="border border-gray-300 px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      {row.notes && (
                        <button
